...
/* compilation command: */
/* compile: gcc -o SAGEM SAGEM2.c -Wall -I/usr/include -lm -lgsl -lgslcblas */
...
void calc_p(double *p,double *rho,double *Z,int nind,int K,float t);
double *init_prior(int K, int nind, int conv, double pl);
int check_conv(double *rho,double *rho_tmp,int nind,int K, double tol);
void matr_copy(double *rho, double *rho_tmp, int conv);
double log_lik(double *logL,int nind,int K);

int main(int argc, char **argv){

  FILE *fp1,*fp2,*fp5,*fprho,*fpm,*fp6;
  char *inp,data[200],zinit[200];
  int ntot,nmax,ploidy,ncnt=0,K,line,nind,xilj,nloci,verbosity,xil,conv,form;
  int git,i,j,k,l,m=1,index,maxit,con,i1,knindi,knindi1;//,flag_l;
  int *X,*allcnt,*Xd,to=0,*id;
  double *Z,*p,*p_tmp,*rho,*yklj,*yklj2,ykl,gamma_lj=1.,gamma_l0,li_ik,up_j,lo_j,pDat2,*logL,*L,pl,lpl,tol,tmpn,la,a,b,tmp_y=0.,logLt,logLt_old=0.,diff;
  float tol2,t,d0,thres,tmpf;
  ...
  fscanf(fp1,"%d%*[^\n] %d%*[^\n] %d%*[^\n] %d%*[^\n] %d%*[^\n] %s%*[^\n] %d%*[^\n] %d%*[^\n] %f%*[^\n] %f%*[^\n] %f%*[^\n] %f%*[^\n] %d%*[^\n] %d%*[^\n] %s%*[^\n] %d%*[^\n]",&ploidy,&form,&line,&nind,&nloci,data,&verbosity,&K,&tol2,&t,&d0,&thres,&maxit,&git,zinit,&con);
  fclose(fp1);
  ...
  // step 0: randomise starting allocation vector
    ...
    if(!(rho = (double *)malloc(conv*sizeof(double))) ||
       !(p = (double *)malloc(conv*sizeof(double)))){ 
      printf("Error in allocating memory in main program\n\n"); 
      return 0;
    }
    ...
    for(i = 0; i < nind; i++)
      for(j = 0; j < K; j++){
	fscanf(fp6,"%f",&tmpf);
	rho[j*nind+i] = (double)tmpf;
	p[j*nind+i]=rho[j*nind+i];
      }
    fclose(fp6);
  }
  /* printf("rho0:\n"); */
  ...
  if(!(yklj = (double *)malloc(nmax*sizeof(double))) ||
     !(Z = (double *)calloc(conv,sizeof(double))) ||
     !(p_tmp = (double *)calloc(conv,sizeof(double))) ||
  ...
  /* m = 0; */
    logLt = 0.;
    printf("SAGEM algorithm iteration number: %d\nE-step...\n",m);
    ncnt = 0;
	...
	knindi1 = k*nind+i1;
	  ...
	  for(j = 0; j < allcnt[l]; j++){
	    ...
	    if(i == 0){
	      //printf("hit2\n");
	      yklj2[index] = y_klj(p,k,nind, Xd, l, j, ploidy); // calculate contribution of all ind in pop k, locus l, allele j
	      /* printf("Prior removing contr of ind 1, pop %d, loci %d, allele %d: yklj2[%d] = %f\n",k+1,l+1,j+1,index,yklj2[index]); */
	    }
	    else tmp_y = update_contribution(p[knindi1],Xd,i1,l,j,ploidy,nind); //-uppdate_contribution(Z_old[knindi1],Xd,i1,l,j,ploidy,nind);//offset(k,l,j,K,nloci) // change the contribution of individual i-1 (because of updated allocation Z[i-1,k])
	    ...
	    yklj2[index] += tmp_y - update_contribution(p[knindi],Xd,i,l,j,ploidy,nind); // remove contribution from ind i
	    //printf("hit3.5\n");
	...
	/* printf("%d ",k); */
	knindi = k*nind+i;
  	rho[knindi] =  L[k]/pDat2;
    	/* diff = sqr(po-rho[knindi]); */
    	/* if(diff/po < tol) ncnt++; */
    	if(verbosity==1) printf("%f ",rho[knindi]);
      }
      /* printf("\n"); */
    ...
    /* show(Z,nind,k); */
    /* printf("old mixture proportion matrix p_tmp:\n"); */
    /* show(p_tmp,nind,k); */
    printf("hit\n");

    Z = s_step(gBaseRand,rho,nind,K,conv,1);
    printf("hit2\n");
    /* printf("stochastic allocation matrix 1 Z:\n"); */
    /* show(Z,nind,k); */
    calc_p(p,rho,Z,nind,K,t);
    printf("check convergence...\n");
    if(con==1){
      ncnt = check_conv(p,p_tmp,nind,K,tol);
      printf("number of converged allocations: %d/%d\n",ncnt,conv);
    }
    logLt = log_lik(logL,nind,K);
    printf("max loglikelihood: %f\n",logLt);


    if((ncnt==conv || m >= maxit) && t < thres && con==1) to = 1;  
    if(con == 2){ 
      diff = sqr(logLt-logLt_old);
      printf("old logL: %f, new logL: %f, diff: %f, tol: %f\n",logLt_old,logLt,diff/logLt,tol);
      logLt_old = logLt;
      if((fabs(diff/logLt) < tol || m >= maxit) && t < thres) to = 1;
    }
    else if(con == 3){
      diff = fabs(logLt-logLt_old);
      printf("old logL: %f, new logL: %f, diff: %f, tol: %f\n",logLt_old,logLt,diff,tol);
      logLt_old = logLt;
      if((diff < tol || m >= maxit) && t < thres) to = 1;
    }
    m++;
    ...
    t *= d0;
    /* printf("GEM allocation matrix rho:\n"); */
    /* show(rho,nind,k);    */

    /* printf("combined allocation matrix p:\n"); */
    /* show(p,nind,k); */

    /* printf("stochastic allocation matrix 2 Z:\n"); */
    /* show(Z,nind,k); */
  }
  ...
  if(m>=maxit)
    printf("WARNING: stopping algorithm before convergence is reached. Perhaps adjusting the tolerance is a good idea. Number of converged allocations: %d/%d\n",ncnt,conv);
  /* recalc_mean(rho,nind,K,nit); */
  if(!(fpm = fopen("model_selection.txt", "w"))){
    printf("Error, could not open model selection save file!\n");
    return 0;
  }
  /* ma = marg_lik_est(fpm,rho,logL,K,nind); */
  /* printf("marginal likelihood: %f\n",ma); */
  la = laplace(fpm,Z,logL,K,nind,nloci);
  printf("Laplace approximation: %f\n",la);
  a = aic(fpm,rho,logL,K,nind,nloci);
  printf("AIC: %f\n",a);
  b = bic(fpm,rho,logL,K,nind,nloci);
  printf("BIC: %f\n",b);
  fclose(fpm);
  if(!(fprho = fopen("mixture_proportion.txt", "w"))){
    printf("Error, could not open mixture proportion save file!\n");
    return 0;
  }
  /* save mean allocations and mixture proportions to files */   

  saveprint(fprho,rho,id,K,nind);
  fclose(fprho);
  free(p_tmp);
  free(id);
  free(p);
...
void matr_copy(double *rho, double *rho_tmp, int conv){
  ...
  for(i = 0; i < conv; i++) rho_tmp[i]=rho[i];
...
int check_conv(double *rho,double *rho_tmp,int nind,int K, double tol){
  ...
  for(i = 0; i < nind; i++)
    for(k = 0; k < K; k++){
      tmp = k*nind+i;
      diff = sqr(rho_tmp[tmp]-rho[tmp]);
      if(diff/rho_tmp[tmp] < tol) ncnt++;
      rho_tmp[tmp] = rho[tmp];
    }
...
double *init_prior(int K, int nind, int conv, double pl){
  ...
  if(!(p = (double *)calloc(conv,sizeof(double)))){ 
    printf("Error in allocating memory in function init_prior\n\n"); 
    return NULL;
  }
  for(i = 0; i < nind; i++)
    for(k = 0; k < K; k++)
...
void calc_p(double *p,double *rho,double *Z,int nind,int K,float t){
  ...
  for(i = 0; i < nind; i++){
    for(k = 0; k < K; k++){
      knind = k*nind;
      p[knind+i] = (double)u*rho[knind+i] + t*Z[knind+i];
    }
  }
}
void recalc_mean(double *Zm,int nind,int K,int nit){
