...
/* compilation command: */
/* compile: gcc -o DAGEM DAEM.c -Wall -I/usr/include -lm -lgsl -lgslcblas */
...
double *init_alloc(const gsl_rng *gBaseRand,int K,int nind); /* initialize allocation vector prior to EM-algorithm */
int find_max(int *allcnt,int nloci);
void saveprint(double *Z, int *id, int K, int nind);
...
int main(int argc, char **argv){
  ...
  FILE *fp1,*fp2,*fp5,*fpm,*fp6;
  char *inp,data[200],zinit[200];
  int ntot,nmax,ploidy,ncnt=0,K,line,nind,xilj,nloci,verbosity,xil,conv,form;
  int git,i,j,k,l,m=1,hit=0,index,i1,knindi,knindi1,ns,maxit,con;//,flag_l;
  int *X,*allcnt,*Xd,*id;
  double *Z,*yklj,*yklj2,ykl,logLmax,gamma_lj=1.,gamma_l0,li_ik,up_j,lo_j,pDat2,*logL,*L,pl,lpl,po,tol,diff,tmpn,la,a,b,tmp_y=0.,tau,logLt,logLt_old=0.;
  float tol2,t,d,tlim,tmpf;
  ...
  fscanf(fp1,"%d%*[^\n] %d%*[^\n] %d%*[^\n] %d%*[^\n] %d%*[^\n] %s%*[^\n] %d%*[^\n] %d%*[^\n] %f%*[^\n] %f%*[^\n] %f%*[^\n] %f%*[^\n] %d%*[^\n] %d%*[^\n] %d%*[^\n] %s%*[^\n] %d%*[^\n]",&ploidy,&form,&line,&nind,&nloci,data,&verbosity,&K,&tol2,&t,&d,&tlim,&ns,&maxit,&git,zinit,&con);
  fclose(fp1);
  ...
  if(!(allcnt = (int *)malloc(nloci*sizeof(int))) ||
     ...
     !(logL = (double *)malloc(K*nind*sizeof(double)))){ 
    printf("Error in allocating memory in main program\n\n"); 
    return 0;
  }
  xil = ploidy;
  ...
  //printf("Initial temperature: %f, temperature limit: %f\nncnt = %d, conv = %d\n",t,tlim,ncnt,conv);
  // step 1: start main loop EM-iterations until convergence
  /* for(m = 0; m < 5; m++){ */
  while(hit==0){ // stop algorithm if convergence is reached and the temperature is low enough
  /* while(m < 2){ */
  /* m = 0; */
    logLt = 0.;
    tau = (double)1./t;
    printf("DAGEM algorithm iteration number: %d\nTemperature: %f, temperature limit: %f\n",m,t,tlim);
    ncnt = 0;
	...
	knindi1 = k*nind+i1;
	  ...
	  /* printf("\n"); */
	logL[knindi] = li_ik;
  	if (verbosity==1) printf("LogLikelihood for data at pop no %d: %f\n",k+1,li_ik);
	...
	/* printf("L[%d] = %f, logL = %f, lpl = %f, tmpn = %f\n",k,L[k],logL[knindi],lpl,tmpn); */
      }
      ...
      for(k = 0; k < K; k++){
	knindi = k*nind+i;
  	po =  pow(L[k],tau)/pDat2;
	if(con==1){
	  diff = sqr(po-Z[knindi]);
	  if(diff/po < tol) ncnt++;
	}
    	Z[knindi] = po;
	...
	hit=1;
      ...
      logLt_old = logLt;
      if((fabs(diff/logLt) < tol && t<=1.) || m >= maxit) hit = 1;
    }
    else if(con == 3){
      diff = fabs(logLt-logLt_old);
      printf("old logL: %f, new logL: %f, diff: %f, tol: %f\n",logLt_old,logLt,diff,tol);
      logLt_old = logLt;
      if((diff < tol && t<=1.) || m >= maxit) hit = 1;
    }
  ...
  if(m==maxit){
    printf("WARNING: stopping algorithm before convergence is reached and/or temperature is not cool enough. Perhaps adjusting convergence parameters is a good idea. Number of converged allocations: %d/%d\n",ncnt,conv);
  }
  la = laplace(fpm,Z,logL,K,nind,nloci);
  printf("Laplace approximation: %f\n",la);
  a = aic(fpm,Z,logL,K,nind,nloci);
  printf("AIC: %f\n",a);
  b = bic(fpm,Z,logL,K,nind,nloci);
  printf("BIC: %f\n",b);
  fclose(fpm);
  saveprint(Z,id,K,nind);
...
double *init_alloc(const gsl_rng *gBaseRand, int K,int nind){
  ...
  if(!(Z = (double *) malloc(K*nind*sizeof(double))) ||
  ...
  for(i = 0; i < nind; i++){
    gsl_ran_multinomial (gBaseRand, K, rep, pr, tmp);
    /* rmultinom(rep, pr, K, tmp); */
    for(k = 0; k < K; k++) Z[k*nind+i] = (double)tmp[k]/rep;
  }
