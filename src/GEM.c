...
/* compilation command: */
/* compile: gcc -o GEM GEM.c -Wall -I/usr/include -lm -lgsl -lgslcblas */
...
double *init_alloc(int K,int nind); /* initialize allocation vector prior to EM-algorithm */
...
int main(int argc, char **argv){
  ...
  FILE *fp1,*fp2,*fp5,*fp6,*fpm;
  char *inp,data[200],zinit[200];
  int ntot,nmax,git,ploidy,ncnt=0,K,line,nind,xilj,nloci,verbosity,xil,conv,form,con;
  int i,j,k,l,m=1,index,i1,knindi,knindi1,maxit,hit=0,ins;//,flag_l;
  int *X,*allcnt,*Xd,*id;
  double *Z,*yklj,logLmax,*yklj2,ykl,gamma_lj=1.,gamma_l0,li_ik,up_j,lo_j,pDat2,*logL,*L,pl,lpl,po,tol,diff,tmpn,la,a,b,tmp_y=0., logLt, logLt_old = 0.;
  float tol2,tmpf;
  ...
  fscanf(fp1,"%d%*[^\n] %d%*[^\n] %d%*[^\n] %d%*[^\n] %d%*[^\n] %s%*[^\n] %d%*[^\n] %d%*[^\n] %f%*[^\n] %d%*[^\n] %d%*[^\n] %s%*[^\n] %d%*[^\n] %d%*[^\n]",&ploidy,&form,&line,&nind,&nloci,data,&verbosity,&K,&tol2,&maxit,&git,zinit,&con,&ins);
  fclose(fp1);
  ...
  /* m = 0; */
    logLt = 0.;
    printf("GEM algorithm iteration number: %d\n",m);
    ncnt = 0;
    ...
    if(con==1) printf("number of converged allocations: %d/%d\n",ncnt,conv);
      ...
      logLt_old = logLt;
      if(fabs(diff/logLt) < tol || m >= maxit) hit = 1;
    }
    else if(con == 3){
      diff = fabs(logLt-logLt_old);
      printf("old logL: %f, new logL: %f, diff: %f, tol: %f\n",logLt_old,logLt,diff,tol);
      logLt_old = logLt;
      if(diff < tol || m >= maxit) hit = 1;
    }
    if(con == 1 && (ncnt>=conv || m >= maxit)) hit = 1;
    m++;
  ...
  /* ma = marg_lik_est(fpm,Z,logL,K,nind); */
...
double *init_alloc(int K,int nind){
